...
void applyHueSaturation(vector< vector<Pixel> > &imageVector,float &saturationValue, float &hueValue)
{
    ...
    for(int j = 0; j < imageHeight; j++)
    {
        ...
        {
            float l =(max({imageVector[i][j].r,imageVector[i][j].g,imageVector[i][j].b})+min({imageVector[i][j].r,imageVector[i][j].g,imageVector[i][j].b}))/200.0;
            ...
            imageVector[i][j].r= (min(255,(int)(abs(r+m) * 255)));
            imageVector[i][j].g= (min(255,(int)(abs(g+m) * 255)));
            imageVector[i][j].b= (min(255,(int)(abs(b+m) * 255)));
        }
    }
}